...
const ComplaintRegistration = ({ onBack }: ComplaintRegistrationProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const assignedWorkerId: string | null = null
          ...
          contents: [
            {
              parts: [
                {
                  text: `You are an AI assistant for "Nagar Rakshak", a civic issue reporting app.
Analyze the provided image for a complaint about "${formData.issueType || 'General'}".
...
Image URL: ${publicUrl}`
      ...
      // Parse JSON-only response robustly
      const raw = analysisResult?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || ''
      let parsed: { is_relevant?: boolean; reason?: string; description?: string } | null = null
      try {
      ...
      if (parsed?.is_relevant && parsed.description) {
        setFormData(prev => ({ ...prev, description: parsed.description! }))
        toast({
          title: "AI Analysis Complete",
          description: "Image is relevant. Description added.",
          variant: "default"
        })
      } else if (parsed && parsed.is_relevant === false) {
        toast({
          title: "Image may not match selected issue",
          description: parsed.reason || "Please upload a relevant image or edit the description manually.",
          variant: "destructive"
        })
      } else {
        toast({
